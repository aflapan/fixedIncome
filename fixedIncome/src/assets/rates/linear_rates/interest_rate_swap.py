...
class OvernightIndexSwap(CashflowCollection):
    def __init__(self,
                 float_index: CurveIndex,
                 direction: InterestRateSwapDirection,
                 fixed_rate: float,
                 ...
                 fixed_leg_day_count_convention: DayCountConvention,
                 holiday_calendar: dict[str, Holiday],
                 fixing_date_for_accrual_period: SettlementConvention = SettlementConvention.T_MINUS_ZERO_BUSINESS,
                 payment_delay: SettlementConvention = SettlementConvention.T_PLUS_TWO_BUSINESS,
                 business_day_adjustment: BusinessDayAdjustment = BusinessDayAdjustment.MODIFIED_FOLLOWING
                 ) -> None:
        self._float_index = float_index
    ...
    def generate_floating_leg_accrual_schedule(self) -> list[SwapAccrual]:
        ...
        match self.floating_leg_payment_frequency:
            ...
            case _:
                raise ValueError(
                    f'Payment Frequency {self.floating_leg_payment_frequency} is not valid to generate a floating leg payment schedule.')
...
if __name__ == '__main__':
    ...
    # Calculate both the geometric and arithmetic average rate
    start_date = date(2024, 1, 2)
    end_date = date(2044, 1, 1)
    start_val = KnotValuePair(start_date, 0.05)
    end_val = KnotValuePair(end_date, 0.05)
    test_curve = Curve([start_val, end_val],
                       interpolation_method=InterpolationMethod.LINEAR,
                       interpolation_day_count_convention=DayCountConvention.ACTUAL_OVER_360)

    dates = Scheduler.generate_business_days(start_date, end_date, US_FEDERAL_HOLIDAYS)

    annualized_rates = [test_sofr_swap.calculate_annualized_rate(interest_rate=test_curve, start_date=start_date, end_date=date_obj) for date_obj in dates[1:]]

    plt.figure(figsize=(10, 6))
    plt.plot(dates[1:], annualized_rates)
    plt.xlabel('Date')
    plt.title('Annualized Rate Computed via Compounded Accruals')
    plt.show()