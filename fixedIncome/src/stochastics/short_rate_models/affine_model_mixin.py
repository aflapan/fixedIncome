...
class AffineModelMixin:
    ...
    @abstractmethod
    def time_t_zero_coupon_bond_price(self, price_datetime: date | datetime, maturity_date: date) -> float:
        """
        Calculates the price of a zero coupon bond.
        """

    def time_t_yield(self,
                     yield_datetime: date | datetime,
                     maturity_date: date,
        ...
        """
        time_t_price = self.time_t_zero_coupon_bond_price(price_date=yield_datetime, maturity_date=maturity_date)
        accrual = DayCountCalculator.compute_accrual_length(yield_datetime, maturity_date, day_count_convention)
        return -math.log(time_t_price) / accrual


    @abstractmethod
    def time_t_instantaneous_forward_rate(self, forward_rate_datetime: date | datetime) -> float:
        """
